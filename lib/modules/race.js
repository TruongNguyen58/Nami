...
function createRace(server, io){
	...
	function startAIPlay() {
		...
		return setTimeout(function() {
			...
			broadcast (sessions, 'notification', safe_name +" solved this quest faster than you!"); //msg to rest of players. someone else won!
	...
	socket.on('connection', function (client) {
		...
		client.on('solve_operation', function (data) {
			if (data.operation == operation.solution){
				...
				broadcast (sessions, 'notification', safe_name +" solved this quest faster than you!", client.id); //msg to rest of players. someone else won!
				scores[safe_name] = (scores[safe_name] || 0) + 1 //credit score to client
				emit(client, 'score', scores[safe_name]);
				newOperation();
			}
			else //baaaad. you need some math classes
				emit(client, 'notification', "Another player solved this quest faster than you!" );
		});
	});
}
