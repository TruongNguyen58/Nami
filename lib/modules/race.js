...
function getOnlineNumber() {
	if(numberOnline == 0)
		numberOnline = getRandomInt(500, 1000);
	return numberOnline;
}
...
// function Operation () {
// 	var self = this;
// 	var member1 = getRandomInt(2,12);
// 	var member2 = getRandomInt(2,12);
// 	var member3 = member2 * member1;
// 	var member4 = getRandomInt(2,22);

// 	var operation_type1 = getRandomBoolean() ? '+' : '-'; //build the match challenge.
// 	var operation_type2 = getRandomBoolean() ? '*' : '/'; //build the match challenge.
// 	if(operation_type2 == '/') {
// 		self.quest =  member3 + operation_type2 + member2 + operation_type1 + member4;
// 		self.solution = Math.round(eval(self.quest));
// 	}
// 	else {
// 		self.quest =  member1 + operation_type1 + member2 + operation_type2 + member3;
// 		self.solution = Math.round(eval(self.quest));
// 	}
...
function Operation () {
	var self = this;
	var div1 = getRandomInt(2,9);
	var div2 = getRandomInt(2,9);
	var totalDiv = div1*div2;
	var member3 = getRandomInt(2,9);
	var member4 = getRandomInt(2,9);
	var member5 = getRandomInt(2,9);
	var operation1 = "+";
	var operation2 = "-";
	var operation3 = "*";
	var operation4 = "/";
	...
	if(r%4 == 0) {
		self.quest = totalDiv+operation4+ div1 +operation1+ member3+operation3+member4 +operation2+ member5;
	}else if(r%4 == 1) {
		self.quest = totalDiv+operation4+div1 +operation3+ member3+operation1+member4 +operation2+ member5;
	}else if(r%4 == 2) {
		self.quest = totalDiv+operation4+div1 +operation2+ member3+operation1+member4 +operation3+ member5;
	}else if(r%4 == 3) {
		self.quest = totalDiv+operation4+div1 +operation1+ member3+operation3+member4 +operation1+ member5;
	}
	self.solution = Math.round(eval(self.quest));	
}
...
function getNumberUserOnlines(req, res) {
	...
	var number = {"n" : (getOnlineNumber() + sessions.length)};
	...
	socket.on('connection', function (client) {
		...
		client.on('solve_operation', function (data) {
			if (data.operation == operation.solution){
				...
				emit(client, 'notification', "Good!");//msg to winner
